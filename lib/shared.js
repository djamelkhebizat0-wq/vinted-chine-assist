...
(function (root) {
  ...
  VCA.KEYS = {
    ...
    autoState: "vca_auto_state",
    autoLog: "vca_auto_log"
  };
  ...
  /** Mode auto local : OFF si le worker cloud est activé (XOR, évite les doubles envois). */
  VCA.localAutoActive = function localAutoActive(settings) {
    return !!(settings?.modeAuto && !settings?.cloudEnabled);
  };
  ...
  VCA.loadAll = async function loadAll() {
    ...
    const autoState = data[VCA.KEYS.autoState] || null;
    const autoLog = Array.isArray(data[VCA.KEYS.autoLog]) ? data[VCA.KEYS.autoLog] : [];
    return {
      settings,
      templates,
      crm,
